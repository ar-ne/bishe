...
import {VolumeConfig, ExtVolInspectInfo, VolumeConfigType} from '../config.d/volume.config'
import Log from '../log'
...
export default class Volume extends Command {
  ...
  async do(volume: ExtVolInspectInfo, fullPath: string, action: ACTION) {
    ...
    const rStr = this.randomStr()
    const dockerCMD = `docker run --rm -v ${dir}:/host-${rStr} -v ${volume.Name}:/target-${rStr} ubuntu`
    let tarCMD = ''
    switch (action) {
      case ACTION.e:
        tarCMD = `tar -cvpf  /host-${rStr}/${filename} /target-${rStr}`
        break
      case ACTION.i:
        tarCMD = `tar --same-owner -xvpf  /host-${rStr}/${filename} /target-${rStr}`
        break
    }
  ...
  randomStr() {
    ...
    for (let i = 0; i < 8; i++) result += c.charAt(Math.floor(Math.random() * c.length))
    return result
  }
  ...
  async run() {
    ...
    const allVolumeList: ExtVolInspectInfo[] = await docker.listVolumes().then(v => v.Volumes)
    ...
    if (!targets || targets.length === 0) Log.error('Nothing to do/show')
    console.table(
      targets.map(v => ({
